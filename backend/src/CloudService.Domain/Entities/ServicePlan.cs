...
public sealed class ServicePlan : AuditableEntity
{
        ...
        int displayOrder)
    {
        ...
        if (cpuCores is <= 0) throw new ArgumentOutOfRangeException(nameof(cpuCores));
        if (ramGb is <= 0) throw new ArgumentOutOfRangeException(nameof(ramGb));
        if (storageGb is <= 0) throw new ArgumentOutOfRangeException(nameof(storageGb));
        if (bandwidthGb is <= 0) throw new ArgumentOutOfRangeException(nameof(bandwidthGb));
        ...
        RamGb = ramGb;
        StorageGb = storageGb;
        StorageType = string.IsNullOrWhiteSpace(storageType) ? null : storageType.Trim();
    ...
    public void SetQrCode(string targetUrl, string qrCodePath, DateTime generatedAt)
    {
        QrTargetUrl = Guard.Required(targetUrl, nameof(targetUrl));
        QrCodePath = Guard.Required(qrCodePath, nameof(qrCodePath));
